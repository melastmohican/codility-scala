...
object CountDistinctSlices {
  ...
  def solution(m: Int, a: Array[Int]): Int = {
    ...
    val used: Array[Boolean] = Array.ofDim[Boolean](m + 1)
    var result: Int = 0
    var back: Int = 0
    for (front <- 0 until a.length) {
      while (used(a(front))) {
        used(a(back)) = false
        back += 1
      }
  ...
  // 100/90
  def solution1(m: Int, a: Array[Int]): Int = {
    var result: Int = 0
    var front: Int = 0
    var back: Int = 0
    val seen: Array[Boolean] = Array.ofDim[Boolean](m + 1)
