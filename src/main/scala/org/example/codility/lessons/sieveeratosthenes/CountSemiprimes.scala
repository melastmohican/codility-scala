...
object CountSemiprimes {
  ...
  def solution(n: Int, p: Array[Int], q: Array[Int]): Array[Int] = {
    var m = p.length
    var factors = Array.ofDim[Int](n + 1)
    val cumSemiprimes = Array.ofDim[Int](n + 1)
    val countSemiprimes = Array.ofDim[Int](m)
    ...
    for (i <- 4 to n) {
      ...
      if (factors(i) != 0) if (factors(i / factors(i)) == 0) cumSemiprimes(i) += 1
    }

    for (i <- 0 until m) {
      countSemiprimes(i) = cumSemiprimes(q(i)) - cumSemiprimes(p(i) - 1)
    }
